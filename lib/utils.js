...
export function compact(
  ...
  /** Use this X when calculating the compact */
  targetItem: { i: string, originalX: number, newX: number }
): { layout: Layout, shadowLayout: Layout } {
  const { i, originalX, newX } = targetItem || {};
  ...
  // Get original order
  const originalOrder = sorted.map(({ i }) => i);

  if (i) {
    /**
     * Order the array so that the item we are moving is pushing other
     * items downwards instead of the item in question would jump around.
     */
    const layoutItem = sorted.find(item => item.i === i);
    sorted = sorted.filter(item => item.i !== i);
    const index = sorted.findIndex(
      item => item.y + item.h >= layoutItem.y + layoutItem.h
    );
    sorted.splice(index, 0, layoutItem);
  }

  sorted.forEach(item => {
    let l = cloneLayoutItem(item);

    l.x = item.i === i && typeof newX !== "undefined" ? newX : l.x;
   ...
   * Restore order
   */
  const outInOriginalOrder = out.sort(
    (a, b) => originalOrder.indexOf(a.id) - originalOrder.indexOf(b.id)
  );

  /**
   * Restore original X to the layout item
   * This will prevent the layout item being actually moved
   */
  const originalWithInitialX = outInOriginalOrder.map(item => {
    if (item.i === i) {
      return {
        ...item,
        x: originalX
  ...
  return { layout: originalWithInitialX, shadowLayout: outInOriginalOrder };
}
...
export function synchronizeLayoutWithChildren(
  ...
  const { layout: compactLayout } = compact(correctedLayout, compactType, cols);
  return allowOverlap ? correctedLayout : compactLayout;
}
