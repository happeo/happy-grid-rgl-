...
export function compact(
  ...
  if (i) {
    ...
    sorted.unshift(layoutItem);
  }

  sorted.forEach(item => {
    let l = cloneLayoutItem(item);

    l.x = item.i === i && typeof useX !== "undefined" ? useX : l.x;
    console.log("LX", l.x, useX);
  ...
  const outInOriginalOrder = out.sort((a, b) => {
    return originalOrder.indexOf(a.id) - originalOrder.indexOf(b.id);
  });

  console.log("IN", layout, "OUT", out, "ORIGINAL", outInOriginalOrder);
  return outInOriginalOrder;
}
